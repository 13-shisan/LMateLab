...
export default function StructureBuilder({ preparedStructure = null }) {
  ...
  function openReview() {
    if (!result || !selected?.workflow_compatible) return;
    setReview({
      template: 'mos2_v1', task: 'full_pipeline', poscar: result.files.POSCAR,
      kpoints: initialMeshes(result.kpoints.mesh), incar: structuredClone(DEFAULT_INCAR),
    });
  }
  ...
  return (
    <section className="competition-structure-builder" aria-labelledby="curated-structure-title">
      <div className="competition-structure-builder-heading"><div><span><Box size={15} /> 受控样例结构</span><h2 id="curated-structure-title">结构与 VASP 输入构建</h2></div><p>ASE 3.23.0 · 样例目录 · 最多 200 原子</p></div>
      <div className="competition-structure-builder-grid">
        <form onSubmit={build}>
          {catalog.status === 'loading' ? <CompetitionState status="loading" /> : null}
          <label>材料<select value={parameters.material_id} onChange={(event) => update('material_id', event.target.value)} disabled={!operator || building}>{(catalog.data?.items || []).map((item) => <option key={item.id} value={item.id}>{item.name} · {item.formula}</option>)}</select></label>
          <fieldset><legend>面内超胞</legend><label>a 方向<input type="number" min="1" max="6" value={parameters.repeat_a} onChange={(event) => update('repeat_a', event.target.value)} /></label><label>b 方向<input type="number" min="1" max="6" value={parameters.repeat_b} onChange={(event) => update('repeat_b', event.target.value)} /></label></fieldset>
          <fieldset><legend>层数与真空层</legend><label>层数<input type="number" min="1" max="8" value={parameters.layers} onChange={(event) => update('layers', event.target.value)} /></label><label>真空层 (Å)<input type="number" min="8" max="40" value={parameters.vacuum_angstrom} onChange={(event) => update('vacuum_angstrom', event.target.value)} /></label></fieldset>
          <fieldset><legend>晶胞调整</legend><label>层间距 (Å)<input type="number" min="3" max="10" step="0.1" value={parameters.interlayer_spacing_angstrom} onChange={(event) => update('interlayer_spacing_angstrom', event.target.value)} /></label><label>面内应变 (%)<input type="number" min="-5" max="5" step="0.1" value={parameters.strain_percent} onChange={(event) => update('strain_percent', event.target.value)} /></label></fieldset>
          <label>KPOINTS 来源<select value={parameters.kpoints_source} onChange={(event) => update('kpoints_source', event.target.value)}><option value="mock_qoder">Qoder 判定（本地 mock 建议）</option><option value="manual">用户指定</option></select></label>
          ...
          <div className="competition-structure-builder-actions"><button type="submit" disabled={!operator || building}>{building ? '构建中' : '生成 POSCAR / KPOINTS'}</button><button type="button" onClick={download} disabled={!operator || downloading}><Download size={16} />下载 ZIP</button><button type="button" onClick={openReview} disabled={!operator || !result || !selected?.workflow_compatible}><CheckCircle2 size={16} />核验</button></div>
          {!operator ? <p>Viewer 只能浏览目录。</p> : null}{error ? <p className="competition-agent-error" role="alert">{error}</p> : null}
        </form>
        <div className="competition-structure-preview" aria-live="polite">{result?.structure ? <VaspStructureViewer structure={result.structure} /> : <div className="competition-structure-empty">选择参数后构建结构预览</div>}<div className="competition-structure-summary-line"><strong>{result?.summary?.formula || selected?.formula || '—'}</strong><span>{result?.summary?.atom_count ?? '—'} 原子</span><span>{result?.summary?.layers ?? parameters.layers} 层</span><span>{selected?.workflow_compatible ? '可进入受控工作流' : '仅构建与下载'}</span></div>{result ? <div className="competition-structure-files">{Object.keys(result.files || {}).map((name) => <code key={name}>{name}</code>)}</div> : null}<p>POTCAR 不包含在输入包中；Qoder 网格仅为建议，必须人工核验。</p></div>
      </div>
