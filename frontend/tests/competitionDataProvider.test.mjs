...
test('demo workflows preserve the preview status and step order', async () => {
  ...
  assert.deepEqual(workflows.map(({ status }) => status), ['succeeded', 'running', 'failed']);
  assert.deepEqual(workflows[0].steps.map(({ key }) => key), ['relax', 'scf', 'band', 'dos']);
  assert.equal(workflows.every(({ data_kind }) => data_kind === 'demo'), true);
});
...
test('demo database pagination is stable and reports the full total', async () => {
  ...
  assert.equal(new Set([...firstPage.rows, ...secondPage.rows].map(({ id }) => id)).size, 3);
});
