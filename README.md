...
Agent 工作区包含普通问答、文献检索与 PDF 索引、VASP 计算目录分析、结构库检索、计算规划和受控
Qoder 接口。Qoder 只通过五个应用内只读工具搜索 QMOF/已上传结构、分析已挂载 VASP 结果与脱敏工作流、
读取确定性计算草案；它不能访问 Shell、任意文件、Slurm 或 VASP 执行接口。用户检查后仍须由 LMateLab
校验并显式提交工作流。API Key、Qoder 登录状态和用户上传内容只保存在部署机私密目录，不写入 Git。
