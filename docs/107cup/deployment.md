...
当前稳定服务是 Job `54393`、`anode18:18731`、提交 `de612a8314cdd2b5acdda72b726e3cbc211a85e9`，release manifest SHA-256 为 `741d274dac4a7d5b925ab1c5ede46e16e75e57ca45c9e7188fdd8c58fa53acfa`；配套 Agent Worker 是 Job `54395/anode18`，两者的提交和 manifest 一致。正式构建 Job `54391/anode01` 为 `COMPLETED/0:0`，后端 `602` 项通过（另有 4 项环境跳过）、前端 `159/159`，Vite 转换 `1871` 个模块。107 内部、4090 relay 和 Windows 公网 `live/ready` 均返回新身份；旧 Web `54338/anode20` 与旧 Worker `54305/anode16` 在逐项核对归属后受控停止，Web 日志包含完整 Uvicorn shutdown，未触及共享账号下其他作业。登录后的 Operator/Viewer 浏览器复核、真实 API Key 和 Qoder CN 登录仍需单独完成，不能由机器健康检查替代。下一次服务发布仍必须重新走 Slurm 构建、候选服务、relay 切换和只读验收，不能沿用本组 Job 冒充新版本证据。
