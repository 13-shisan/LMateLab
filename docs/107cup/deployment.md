...
当前已验证稳定服务是 Job `46107`、`anode19:18731`、提交 `6012b2bbb976a534e4ca18e93bfe1eb0a4dc6aec`，release manifest SHA-256 为 `fded9e2b4cd14d049c244e9185597432afc3d561025373dca970929f47a6e4b9`。正式构建 Job `46105/anode01` 为 `COMPLETED/0:0`；4090、Windows Operator 和公网 `live/ready` 均返回新身份；Stage 10 只读 Job `46109/anode19` 为 `COMPLETED/0:0`，输出 `STAGE10_ACCEPTANCE_OK` 且 stderr 为 0 字节。旧 Job `43737/anode16` 经完整归属核验后已受控停止，旧端口不可达，未触及共享账号下其他作业。下一次服务发布仍必须重新走 Slurm 构建、候选服务、relay 切换和只读验收，不能沿用本组 Job 冒充新版本证据。

PR #79 已于 `2026-08-31` 把公网 Operator 精确写入门禁合并为 `cf813fe9564b2d29864acce089d21413547b3d94`。本次只更新 4090 Nginx，不需要重建或重启 FastAPI release；服务仍为 Job `46107/anode19`。活动配置 `/home/Pwjb/.config/lmatelab-107cup-proxy/conf/nginx.conf` 为 `0600`，SHA-256 为 `88b62e7331dda744a0d0ede7c94921eeb197a6b83af0667105ed1932816c6b6b`；旧配置备份为 `backups/nginx.conf.before-public-operator-write.20260831T035304Z`，SHA-256 为 `6c6dafe1d3091684e372a3431b562a5d05a829e499479e88ba97bd7f377211dc`。候选、替换前临时文件和活动配置均通过 Nginx `1.18.0` 的 `nginx -t`，reload 后 `live/ready` 身份不变。
