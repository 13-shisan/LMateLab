...
当前稳定服务是 Job `54595`、`anode16:18731`、提交 `4a240dfd132dacc449d733e60a631654b1db1d8f`，release manifest SHA-256 为 `f82395a52fd30d8ed91c3d3ffeaa185278246f18ebc8786cbbe5a73361a70d3f`；配套 Agent Worker 是 Job `54612/anode18`，两者的提交和 manifest 一致。正式构建 Job `54573/anode01` 通过后端 `602` 项（另有 4 项环境跳过）、前端 `159/159`、Vite `1871` 个模块和双重清单门禁。107 内部、4090 relay、Windows 公网与回环入口的 `live/ready` 均返回新身份；旧 Web `54304/54328/54393` 与旧 Worker `54395` 在逐项核对归属后受控停止，Web 日志包含完整 Uvicorn shutdown，最终只保留一组 Web/Worker。4090 Nginx 活动配置 SHA-256 为 `052eedc932a07d27581c1892cfc200feb41f654199399f1a33c7dcd8e3054607`，替换前备份为 `backups/nginx.conf.before-allowlisted-api-key.54573`、SHA-256 `8086408cfa0b9a3b238545a48662acdade4310d8c606aa08125ac90cf20e67a4`，两者权限均为 `0600`。已登录 Operator 自行保存真实 API Key 和 Qoder CN 登录仍需单独完成，不能由机器健康检查替代。下一次服务发布仍必须重新走 Slurm 构建、候选服务、relay 切换和只读验收，不能沿用本组 Job 冒充新版本证据。
