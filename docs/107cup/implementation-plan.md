...
- [ ] 合并后通过 107 Slurm 计算节点完成正式构建与测试，启动候选服务并核对新 Job、节点、提交和 manifest；只有新服务健康、真实接口返回并确认公网页面自动刷新后，才能停止旧服务。
- [ ] 完成生产 `fresh`、故障降级和敏感字段不泄漏验收，运行 Stage 10 只读门禁并固化部署证据；完成前本节只能记为“本地实现完成、生产验收待完成”。
