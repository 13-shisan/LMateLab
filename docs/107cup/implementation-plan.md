...
| 3. 最小 107 网页服务 | DONE | PR #52/#53 已部署；当前 Job `41044` 运行发布 `e565851...`，4090 每分钟短守护、maintenance、单候选恢复和 `18742 -> 18740` 先验切换已通过真实停服/恢复验收 | 按运行手册维护；SSH 主连接真实失效时由 Operator 完成一次二次验证 |
...
- 分支 `codex/107cup-public-home` 把公开根路径 `/` 从登录重定向改为 107 杯项目首页，`/login` 保持为认证入口；登录页导航同步指向首页的平台、工作流和证据区段。
- 首页只介绍固定 MoS2、`relax -> SCF -> BAND -> DOS`、Slurm 归属与结构/BAND/DOS 证据，不引入 Agent、机器学习、QE/EPW、任意材料或任意命令。用户提供的樱花校园图用于全幅首屏，湖景图用于第二个全宽平台区段。
- Windows 本地门禁通过前端 `132/132`、聚焦首页/角色合同 `8/8`、定向 ESLint 和 107 Cup live Vite 构建（`1861` 个模块）。Playwright 在 `1440x900` 与 `390x844` 验证两张背景已加载、页面零横向溢出、控制台错误为 0；本地截图位于未纳入 Git 的 `output/playwright/public-home/`。
- 以上仅是未合并分支的本地证据。现有 Viewer/Operator 截图与 `422.04` 秒视频继续严格绑定固定发布 `4f81d727...`，不能用于证明新首页已部署。该分支合并后必须经 107 Slurm 重新构建、部署和桌面/移动浏览器复跑，才能更新固定发布身份；Stage 10 继续为 `PARTIAL`。
