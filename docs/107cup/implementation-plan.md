...
- 功能提交 `f460d7018379c70a29dd8eb67df117e6f066bfa1` 和预检证据提交 `5232c2b` 已通过 PR #95 合并为 `de612a8314cdd2b5acdda72b726e3cbc211a85e9`。正式 `runtime.env` 只新增 `QODERCN_CONFIG_DIR`，原文件以权限 `0600` 做不可覆盖备份，没有写入 API Key 或 PAT。
- 正式构建 Job `54391/P107-RTX5090/anode01` 为 `COMPLETED/0:0`，耗时 `00:01:33`；后端 `602` 项通过（另有 4 项环境跳过）、前端 `159/159`、Vite 转换 `1871` 个模块，release manifest SHA-256 为 `741d274dac4a7d5b925ab1c5ede46e16e75e57ca45c9e7188fdd8c58fa53acfa`。stdout/stderr 已收紧为 `0600`，SHA-256 分别为 `5a9d1c07ca07b2f819672bd5827ddd487c02635201007cdd777a296ab66e150a` 和 `d5bc294f8bc1bb14bd7220e3a6ae2872fbbdf6ab32e8d64b7ae74348d08c91bc`。
- 候选 Web Job `54393/P107-A100/anode18` 为两套 SQLite 创建不可覆盖的迁移前备份，并通过两个 Alembic head、数据库完整性、live/ready、提交和 manifest 身份检查。由于旧 Web `54338` 与旧 Worker `54305` 占满 A100 用户配额，先只读确认 Agent 活动队列为 0，再完整核对并停止旧 Worker；候选启动并通过 4090 临时转发后，公网 relay 返回 `54393/anode18` 的新身份，才停止旧 Web。旧 Web 日志记录完整 Uvicorn shutdown；旧作业已从短时 `scontrol` 记录中消失，保留原始日志作为取消证据。
- 4090 Nginx 活动配置在旧 SHA-256 精确等于 `298b1cb799fba9c44a6638b3711733b67c0fdc57e329a67ebe1a135a61d5ce34` 时才备份和替换。候选与活动配置均通过 Nginx `1.18.0` 语法检查，当前配置权限为 `0600`、SHA-256 为 `8086408cfa0b9a3b238545a48662acdade4310d8c606aa08125ac90cf20e67a4`；备份 `backups/nginx.conf.before-qodercn-54393` 为 `0600` 且保持旧哈希。Windows 公网入口已返回新 live/ready 和 Agent 页面 HTTP 200。
...
- 清单修复由仓库固定生成器完成并通过 PR #98 合并为 `4a240dfd132dacc449d733e60a631654b1db1d8f`。重建 Job `54573/P107-RTX5090/anode01` 通过后端 `602` 项（另有 `4` 项环境跳过）、前端 `159/159`、Vite `1871` 个模块和 release 双重清单，生成 manifest `f82395a52fd30d8ed91c3d3ffeaa185278246f18ebc8786cbbe5a73361a70d3f`。构建 stdout/stderr 已收紧为 `0600`，SHA-256 分别为 `6facd3b433fa3859ce50a2ee632678f3bc706996be847657497d764c95581b4a` 与 `ab29e2db5c0f84812031ecbfd14736fffa9c6f666bfbdda81dc0ba698b2b9747`。
- 4090 Nginx 仅在活动配置精确匹配旧 SHA-256 `8086408cfa0b9a3b238545a48662acdade4310d8c606aa08125ac90cf20e67a4` 后接受新模板；候选和活动配置均通过 Nginx `1.18.0` 语法检查。新活动 SHA-256 为 `052eedc932a07d27581c1892cfc200feb41f654199399f1a33c7dcd8e3054607`，旧配置备份 `backups/nginx.conf.before-allowlisted-api-key.54573` 保持旧哈希，两者权限均为 `0600`。更新 Nginx 时公网仍返回旧 Web `54393`，因此没有提前放宽旧后端。
- 新 Web Job `54595/P107-A100/anode16` 通过迁移前数据库备份、两个 Alembic head、数据库完整性、direct live/ready 以及 commit/manifest 身份门禁后发布 state。4090 恢复器随后把 `18740/18733` 切换到 `54595`；Windows 公网与回环入口均返回同一新身份，Agent 动态资源包含 `allowlisted_http` 状态和未加密警告。
- 更换 Worker 前的首次只读检查 Job `54604` 因 `sbatch --wrap` 引号错误失败，SQLite 把 `queued` 误作列名；数据库没有修改，stdout 为空，stderr SHA-256 为 `5bdc8535756353478a7b8f58d0bae0c549768339eeb7b0ddef4565e568e35c6f`。使用 Base64 参数化查询的新 Job `54610` 返回 `ACTIVE_AGENT_RUNS=[]`，stderr 为空，stdout SHA-256 为 `b75a432e6eff911cbe1742df4ce69f5be2f80e90718a5c1351060578042350fd`；四个日志均为 `0600`。
...
- [x] 本地实现只读接口 `GET /api/competition/cluster-resources`。采集器只执行固定参数命令 `/usr/bin/scontrol --json show nodes anode[01-26]`，只接受 `P107-RTX5090`、`P107-A100` 与 `anode01..anode26`，并返回节点状态、CPU、内存及调度器实际提供的 GPU GRES 分配/空闲数据。
- [x] 采集设置 5 秒超时、1 MiB 输出上限和 10 秒进程内缓存。刷新失败且存在成功快照时返回 `stale`，首次失败返回 `unavailable`；GPU 动态数据缺失时明确返回不可用，不以配置总量或 LMateLab attempt 数推断。响应不包含用户、账号、节点地址、作业命令、工作目录或非竞赛节点。
- [x] “服务器”页面每 15 秒刷新，展示集群及两个竞赛分区汇总、26 个节点的 GPU/CPU/内存明细和 `fresh/stale/unavailable` 状态。页面明确说明当前有剩余资源不等于作业立即启动，仍受 QoS、资源请求和排队优先级影响。
- [x] 本地后端定向测试 `36/36`、前端全量测试 `161/161`、变更文件定向 ESLint、107 Cup 生产构建和 `git diff --check` 均通过；Vite 转换 `1871` 个模块。Playwright 在 `1440x900` 与 `390x844` 检查 26 行表格、状态标识和滚动边界：手机页面本身无横向溢出，节点表格在独立区域横向和纵向滚动，控制台错误与警告均为 0。截图保存在未跟踪目录 `frontend/output/playwright/`。
- [x] 通过 107 短时命令取得真实解析快照：当时 26 个节点中 25 个有可分配 CPU 与 GPU，空闲 GPU 共 169 张；其中 `P107-RTX5090` 为 15 个可用节点、109 张空闲 GPU，`P107-A100` 为 10 个可用节点、60 张空闲 GPU。该结果只是瞬时验证，不作为当前资源承诺。
- [x] PR #102 已合并为 `d0bb2a5b1309b89ec07d2f2e48e5fba5ab1d8527`。Slurm 构建 Job `55036/anode01` 通过后端 `611` 项（另有 4 项环境跳过）、前端 `161/161` 和 Vite `1871` 模块；release manifest 含 `748` 项，SHA-256 为 `bb4236da973996da7c09f84d297db80854f8fee76fc806de5651cb8df4bd6aaf`。新 Web Job `55053/anode18` 通过数据库备份、迁移、内部 live/ready 和身份核对后发布，4090 relay 与公网 `18733` 返回同一 Job、节点、提交和 manifest。
...
- Job `63303/anode16` 把当前认证目录复制到计算节点临时目录后，在该副本上再次执行固定
  `qoderclicn login`，成功取得新的大陆版设备授权流程；原生产认证目录未修改。正式换号因此复用同一固定
  登录命令，不删除认证目录、不暴露凭据，也不引入 Shell 或任意命令输入。
- 新增 Operator 专用 `/api/competition/agent/qoder/account/switch` 固定写接口和公网精确代理规则。远程控制
  服务运行时拒绝换号；授权未完成时 Qoder 引擎保持不可用、新请求不得排队、远程控制服务不得启动。页面
  显示当前 CLI 账号，提供确认后的“切换账号”，并在设备授权期间自动轮询状态。
...
### 17.41 校园网 CIDR 白名单扩展

- 按比赛团队的校园网访问需求，4090 用户态 Nginx 的 `18733` 入口新增 11 个明确 IPv4 CIDR：
  `114.214.160.0/19`、`114.214.192.0/18`、`202.38.64.0/19`、`210.45.64.0/20`、
  `210.45.112.0/20`、`211.86.144.0/20`、`222.195.64.0/19`、`210.72.22.0/24`、
  `202.141.160.0/19`、`218.22.21.0/27` 和 `218.104.71.160/28`。
- 保留原有单 IP、`211.86.0.0/16`、回环地址与最终 `deny all`。新网段只扩展能够到达入口的来源，
  不开放注册，不改变 Operator/Viewer 角色、本人任务归属、Agent 或工作流精确方法门禁。
- 合同测试必须确认 11 个 CIDR 各出现一次且全部位于 server 级 `deny all` 之前。生产更新只允许在旧配置
  SHA-256 精确匹配、候选 `nginx -t` 通过、新配置备份完成后原子替换并 reload；更新后必须再次核对公网
  `live/ready` 身份不变。

### 17.42 DOS 动态 KPOINTS 科学验收修复

- 工作流 `6853dc6d-7e7b-4a55-8732-6c57abcc77ac` 的 DOS Job `63072` 与重试 Job `63082`
  均为 Slurm `COMPLETED/0:0`、VASP 退出码 `0`、电子收敛通过，并生成完整 `OUTCAR`、
  `vasprun.xml` 和 `DOSCAR`；两次科学验收都以 `dos_kpoints_invalid` 失败。
- 根因是受控草案/人工复核接口允许为 `relax/scf/dos` 固化 `1..60` 的 Gamma 网格，本次 DOS 输入为
  `16 10 1`；旧验收器却仍只接受模板默认 `24 24 1` 的单一文件哈希。直接重试不会改变该输入，因而
  第二次重复失败。
- 验收器改为解析规范五行 Gamma 自动网格，逐轴只接受整数 `1..60`。协调器必须从当前 attempt 唯一的
  `attempt_input/KPOINTS` 账本读取已发布 SHA-256，并要求运行目录中的 KPOINTS 与之完全一致；缺失、
  账本字段不一致、运行后替换、非 Gamma、偏移非零、额外行、零值、越界值和超长整数全部失败关闭。
