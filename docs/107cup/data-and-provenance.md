...
v18 可能不同，因此107只声明采用可复核的最新 v18，不宣称与旧运行库逐字节一致。实际107后端按与
页面相同的元素解析规则得到 Mn 结构 `723` 条，与旧截图一致；这支持两端使用相同公开版本，但不是旧库
逐字节同一性的证明。正式安装、失败边界、Job 和哈希见 [`qmof-library-evidence.md`](./qmof-library-evidence.md)。
