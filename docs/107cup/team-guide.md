...
- Operator：Windows 到 107 的独立 SSH 隧道，适合需要写权限的受控验收；
- Viewer：4090 用户态 Nginx 只读代理和 IP 白名单，适合队友/评委查看。

4090 入口不是永久依赖。转发失效时，项目数据、源码、服务和 VASP 作业仍应全部留在 107；只需重新建立隧道或替换只读入口。
